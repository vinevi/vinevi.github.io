...
```map mapname``` — warps you to the selected map
...
```exec anox``` — runs "anoxdata/configs/anox.cfg"*
...
Playing cinematics that way will show a lot of extra things on screen, you will be able to see how the cinematics were put together. This engine module was used to make all cutscenes in the game and left for modders to use. Similarly as for the previous approach you will need to first use the ```map``` command to load the proper environment.
